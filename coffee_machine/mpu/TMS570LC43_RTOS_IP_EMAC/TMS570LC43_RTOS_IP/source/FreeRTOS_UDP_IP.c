        ...
        if( xNetworkBuffersInitialise() == pdPASS )
        {
                if( xNetworkEventQueue != NULL )
                {
                        ...
                        if( xReturn == pdFALSE )
                        {
                                ...
                                // SGH Change made as described in http://www.freertos.org/FreeRTOS_Support_Forum_Archive/March_2015/freertos_Software_Timers_in_FreeRTOS_v8.2_1fca9483j.html
                xReturn = xTaskCreate( prvIPTask, "UDP/IP", ipconfigUDP_TASK_STACK_SIZE_WORDS, NULL, ipconfigUDP_TASK_PRIORITY | portPRIVILEGE_BIT, NULL );
                        }
                }
        }
