...
//weight sensor
volatile uint32 weight,weight_zero;
...
{
    struct udp_pcb *pcb;
    char msg[] = "remote_now = aaaaaa, step = aaaaaa, weight_zero = aaaaaa, drop_count = aaaaaa, drop_time_avg = aaaaaa, temperature = aaaaaa, time_gap_set = aaaaaa, time_left = aaaaaa, ontime = aaaaaa";
    struct pbuf *p;
    err_t err;
    ...
    {
        taskENTER_CRITICAL();
        sprintf(msg, "remote_now = %d, step = %d, weight = %d, weight_zero = %d, drop_count = %d, drop_time_avg = %d, temperature = %f, time_gap_set = %d, time_left = %f, ontime = %d \r\n",remote_now, step, weight, weight_zero, drop_count, drop_time_avg, temp, time_gap_set, time_left, ontime);
 ...
 //        xSemaphoreGive(sem1);
    ...
    case 1 :
        if(time > 0xfffffff0){
            time=0;
        ontime=0;
        // if 'time' overflows, we need higher class of 'time' to memorize
        }

        time++;
      ontime = time;            // 100단위가 1초 즉 10ms마다 돌아가는 것을 확인 함.
//        for(j=0;j<1;j++){
