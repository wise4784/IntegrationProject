 ...
 * FreeRTOS-Plus-IO - UART TMS570LS20X31X V0.0.2  (C) 2014 Lovas Szil�rd
	...
	//taskENTER_CRITICAL();
	{
		switch( ulRequest )
		{
			...
			case ioctlUSE_ZERO_COPY_TX_DMA :
				...
				// TODO: Elt�r�s a 20x31x �s a 4357 eszk�z�k API-ja k�z�tt
 ...
 */
// TODO meg�rni processzor f�ggetlenre
...
void vFreeRTOSsci1Interrupt(void)
{
	...
	switch (vec)
	{
	    ...
	    case 9:					// SCI_OE_INT
	    	configASSERT(0);	// Nem kezelt megszak�t�s forr�s hibajelz�se..
	    //break;
	case 11:					// Receive
		// pxUartRxTransferControlStructs[]-ba el van mentve az SCI1 rx Transfer_Control_t strukt�r�ja.
	...
	if(sci == sciREG1 || sci == sciREG2)
	{
		switch(command)
		{
			...
			case SCI_DISABLE_RX_DMA_MODE:
				sci->CLRINT=(1U<<17U);
				break;
				// ide m�g j�hetnek a tov�bbi parancsok...
			default:
				xReturn=pdFALSE;
		}
	}
