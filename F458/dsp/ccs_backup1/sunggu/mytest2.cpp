...
public:
    ...
    int plotLane(cv::Mat inputImage, cv::Mat img_edges,
...
cv::Mat LaneDetector::deNoise(cv::Mat inputImage) {
        ...
        anchor = cv::Point(-1, -1);
        kernel = cv::Mat(3, 3, CV_32F, cv::Scalar(0));
...
//        cv::adaptiveThreshold(output, output, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 11, 13);

        anchor = cv::Point(-1, -1);
        kernel = cv::Mat(1, 3, CV_32F, cv::Scalar(0));
...
std::vector<cv::Point> LaneDetector::regression(
    ...
    cv::Mat inputImage) {
        ...
        double right2_ini_x = ((ini_y - right2_b.y) / right2_m) + right2_b.x;
        double right2_fin_x = ((fin_y - right2_b.y) / right2_m) + right2_b.x;
...
std::string LaneDetector::predictTurn() {
        std::string output;
        double vanish_x;
        double thr_vp = 60; //origianl value = 10;
...
int LaneDetector::plotLane(cv::Mat inputImage, cv::Mat img_edges,
...
#if 1
//        cv::namedWindow("Edge_img", CV_WINDOW_AUTOSIZE);
        cv::imshow("Edge_img", img_edges);
#endif
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        ...
        int i = 0;
#if 0
        frame = cv::imread("right_sample.png", -1);
