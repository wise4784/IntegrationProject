...
void *can_send_func(void * data);
void *can_recv_func(void * data);
...
int main(int argc, char * argv[])
{
	...
	printf("sound_func\n");
	thr_id = pthread_create(&p_thread[1], NULL, sound_func, (void *)&b);
	if(thr_id < 0)
	{
	...
	printf("can_sen_func\n");
	thr_id = pthread_create(&p_thread[2], NULL, can_send_func, (void *)&c);
	if(thr_id < 0)
	{
		perror("can send thread create error:");
		exit(0);
	}
#if 0
	printf("can_recv_func\n");
	thr_id = pthread_create(&p_thread[3], NULL, can_recv_func, (void *)&d);
	if(thr_id < 0)
	{
		perror("can recv thread create error:");
		exit(0);
	}
#endif
...
//  printf("H : %d, L : %d\n", servo_H, servo_L);
//    char data[32] = {0, c1+'0', c2+'0', c3+'0', c4+'0'};
    char data2[32] = {0, '1', '3', '5', '0', '1'};
...
//    printf("dirf = %d\n", dirf);
	...
	for(;;)
	{
	    ...
	    pthread_mutex_lock(&mutex_lock);
	    char data3[32] = {0, dirf + '0'};
	    pthread_mutex_unlock(&mutex_lock);

	    set_bldc_tx_buf(data3, can_tx_buf, 'd');
	    write(usb2can, can_tx_buf, 21);

#if 0
        printf("----------------------data read\n");
	    //printf("c1 = %d, c2 = %d, c3 = %d, c4 = %d\n", c1, c2, c3, c4);
		set_bldc_tx_buf(data, can_tx_buf, 'd');
		//write(usb2can, can_tx_buf, 21);
		sleep(1);

        read(usb2can, can_tx_buf, 21);
        printf("[0] = %d, [1] = %d, [2] = %d, [3] = %d, [4] = %d\n", can_tx_buf[0]
                                         , can_tx_buf[1], can_tx_buf[2], can_tx_buf[3], can_tx_buf[4]);
        sleep(1);

		printf("\ndata read\n");
		n = read(usb2can, can_tx_buf, 5);
		printf("[0] = %c, [1] = %c, [2] = %c, [3] = %c, [4] = %c\n", can_tx_buf[0], can_tx_buf[1], can_tx_buf[2], can_tx_buf[3], can_tx_buf[4]);
		printf("n = %d\n", n);
		sleep(1);
#endif
	}
    ...
    cv::Mat img_result(LaneDetector lanedetector, cv::Mat frame, int i){
        ...
        int line2_flag = 0; // 1 is right
                      ...
                      // std::cout<<"i =  "<<i<<std::endl;
                       ...
                       veloci = std::to_string(i);
                       cv::putText(img_out, veloci, cv::Point(50, 300),
                         cv::FONT_HERSHEY_COMPLEX_SMALL, 2,
