...
int t; // make about 4 sec
int ignore_flag; // in changing, DSP must ignore  stop_flag
                //DSP change 0 after about 4 sec
/*
 * sound area
 */
...
public:
    ...
    cv::Mat plotLane(cv::Mat inputImage,
        ...
        std::string turn, char line2_flag);
};
...
*/
        ...
        //std::string source = "/home/root/integration/final_out.avi";
...
#if 1
        printf("img_func\n");
        thr_id = pthread_create(&p_thread[1], NULL, img_func, (void *)&b);
        if(thr_id < 0){
...
void vcp_can_tx(int can_fd)
{
    ...
    line2_flag_tmp = line2_flag;
    pthread_mutex_unlock(&mutex3);

    char sbuf[32] = {0, '1', '5', '6', '0'};
   //can_tx_buf[14] = '2';
   // first bit must be 0
...
cv::Mat LaneDetector::plotLane(cv::Mat inputImage,
    std::vector<cv::Point> lane, std::string turn, char line2_flag_temp) {
        cv::Mat output;
        ...
        if(line2_flag_temp == '1'){
            cv::putText(output, "Detect Right", cv::Point(70, 240),
                        cv::FONT_HERSHEY_COMPLEX_SMALL, 2,
                        cv::Scalar(0, 70, 0), 1, CV_AA);
        }
        if(line2_flag_temp == '2'){
            cv::putText(output, "Detect Left", cv::Point(70, 240),
                         cv::FONT_HERSHEY_COMPLEX_SMALL, 2,
                         cv::Scalar(0, 70, 00), 1, CV_AA);
        }
