...
public:
    cv::Mat perspect(cv::Mat inputImg);
...
cv::Mat LaneDetector::perspect(cv::Mat inputImg){
        cv::Mat output;

        cv::Point2f src_data[4];
        src_data[0] = cv::Point2f(270, 200);
        src_data[1] = cv::Point2f(350, 200);
        src_data[2] = cv::Point2f(581, 479);
        src_data[3] = cv::Point2f(120, 479);
        cv::Point2f dst_data[4];
        dst_data[0] = cv::Point2f(230, 0);
        dst_data[1] = cv::Point2f(380, 0);
        dst_data[2] = cv::Point2f(380, 479);
        dst_data[3] = cv::Point2f(230, 479);

        this->M = cv::getPerspectiveTransform(src_data, dst_data);
        this->Minv = cv::getPerspectiveTransform(dst_data, src_data);

        cv::warpPerspective(inputImg, output, this->M, inputImg.size());
...
std::vector<cv::Vec4i> LaneDetector::houghLines(cv::Mat img_mask) {
        ...
        HoughLinesP(img_mask, line, 1, CV_PI/180, 20, 50, 6);
...
LaneDetector::lineSeparation(
    std::vector<cv::Vec4i> lines, cv::Mat img_edges) {
        ...
        cv::Point ini;
        cv::Point fini;
        double slope_thresh_mini = 19.10;
        ...
        for (auto i : lines) {
            ...
            0.00001);
                ...
                // minimum degree value is 87
                if (std::abs(slope) > slope_thresh_mini)
                {
...
//      img_right_center =  static_cast<double>(3*(img_edges.cols / 4))-second_bias;
        ...
        while(j < right_lines.size()){
            ...
            if(fini.x < (second_xstandard-35) && ini.x < (second_xstandard-35)){
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        ...
        while (cap.read(frame)) {
                ...
                img_persp = lanedetector.perspect(img_mask);
#endif

#if 1
                lines = lanedetector.houghLines(img_persp);
#endif

#if 1
                cv::Mat draw_img(img_persp.rows, img_persp.cols, CV_8UC3);

                for( int i=0; i<lines.size(); i++ )
                 {
                     cv::Vec4i L = lines[i];
                     line(draw_img, cv::Point(L[0],L[1]), cv::Point(L[2],L[3]),
                          cv::Scalar(0,0,255), 2, cv::LINE_AA );
                 }

                cv::imshow("myline_plot", draw_img);
...
#if 0
                        left_right_lines =
                               lanedetector.lineSeparation(lines, img_persp);
#endif
